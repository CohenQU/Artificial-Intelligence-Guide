...
\lhead{} 
\chead{} 
\rhead{\textsuperscript{\textcopyright} \textbf{2019 Yuxiao QU}} 
...
\indent 2. May not overfit, one reasonable explanation is that these decision tree stumps tend to wrap themselves so tightly around the error points so there is no room for overfitting because nothing else will fit in that same volume.
\newpage
...
\indent 1. Chew away those variables from the bottom and build a linear list of all these variables
$$(P,D,B,T,R)$$
